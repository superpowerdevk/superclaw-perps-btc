...
# SuperClaw Copy-Trade — Interaction Flow (EN)
...
After installing the SuperClaw Copy-Trade skill, the user can — via conversation — automatically follow a centrally-curated trading agent on Hyperliquid. The followed agent is **not chosen by the user**; it is resolved automatically from the official remote pointer at service start/resume.
...
3. Run & manage (status, pause, sync latest agent, stop)
...
1. **Coin scope**: the default supported majors are BTC, ETH, SOL, BNB, APT, ATOM, ARB, AVAX, ADA, BCH, DOGE, DOT, FIL, HBAR, LINK, LTC, NEAR, OP, SUI, TRX, XRP, UNI; controlled by the `allowed_coins` whitelist. An empty list means no restriction.
